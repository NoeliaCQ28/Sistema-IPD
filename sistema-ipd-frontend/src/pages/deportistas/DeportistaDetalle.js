...
const DeportistaDetalle = () => {
    ...
    const formatFecha = (fecha) => {
        if (!fecha) return 'N/A';
        try {
            // Aseguramos que la fecha se interprete correctamente, añadiendo la hora para evitar problemas de zona horaria
    ...
    const getBackPath = () => {
        if (!user) return '/login'; // Fallback por si el usuario no ha cargado

        // Si el rol es ENTRENADOR, vuelve a la vista de "Mis Deportistas"
        if (user.rol === 'ENTRENADOR') {
            return '/portal/entrenador/mis-deportistas';
        }
        
        // Si el rol es ADMINISTRADOR, vuelve al portal de deportistas del dashboard
        if (user.rol === 'ADMINISTRADOR') {
            return '/dashboard/deportistas';
        }

        // Si es otro rol (o por alguna razón no coincide), vuelve a una ruta segura
        return '/login';
    };
    
    // Función para navegar al historial de progreso
    const goToHistorial = () => {
        // La ruta base depende del rol del usuario actual
        const basePath = user.rol === 'ENTRENADOR' ? '/portal/entrenador' : '/dashboard';
        navigate(`${basePath}/deportistas/ver/${deportista.id}/progresos`);
    };
    ...
    return (
        <div className="detail-container">
            ...
            {/* SE HA ELIMINADO EL BLOQUE QUE MOSTRABA EL HISTORIAL AQUÍ */}
